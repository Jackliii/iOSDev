...
+ (instancetype)levelInfoWithLeft:(LOPNode *)left right:(LOPNode *)right {
    LOPLevelInfo *info = [[self alloc] init];
...
+ (instancetype)nodeWithString:(NSString *)string {
    ...
    LOPNode *node = [[self alloc] init];
    node->_string = string;
    node->_width = string.length;
    return node;
}

+ (instancetype)nodeWithBtNode:(id)btNode tree:(id<BinaryTreeInfo>)tree {
    LOPNode *node = [self nodeWithString:[[tree string:btNode] description]];
    node->_btNode = btNode;
    return node;
}
...
- (NSUInteger)treeHeight:(LOPNode *)node {
    if (!node) return 0;
...
- (LOPLevelInfo *)levelInfo:(NSUInteger)level {
    ...
    return [LOPLevelInfo levelInfoWithLeft:left right:right];
}
...
{
    LevelOrderPrinter *printer = [super printerWithTree:tree];
    printer->_root = [LOPNode nodeWithBtNode:[tree rootNode] tree:tree];
...
- (NSString *)printString {
    ...
    for (NSUInteger i = 0; i < rowCount; i++) {
        ...
        for (LOPNode *node in rowNodes) {
            ...
            [rowString appendString:[NSString m_blank:leftSpace]];
...
- (LOPNode *)_addNode:(NSMutableArray *)nodes btNode:(id)btNode {
    LOPNode *node = nil;
    if (btNode) {
        node = [LOPNode nodeWithBtNode:btNode tree:self.tree];
...
- (void)_fillNodes {
    ...
    while (YES) {
        ...
        BOOL notNull = NO;
        for (LOPNode *node in preRowNodes) {
            if ([node isEqual:[NSNull null]]) {
                [rowNodes addObject:[NSNull null]];
                [rowNodes addObject:[NSNull null]];
            } else {
                LOPNode *left = [self _addNode:rowNodes btNode:[self.tree left:node->_btNode]];
                if (left) {
                    node->_left = left;
                    left->_parent = node;
                    notNull = YES;
                ...
                LOPNode *right = [self _addNode:rowNodes btNode:[self.tree right:node->_btNode]];
                if (right) {
                    node->_right = right;
                    right->_parent = node;
                    notNull = YES;
...
- (void)_addXLineNode:(NSMutableArray *)curRow parent:(LOPNode *)parent x:(NSUInteger)x {
    LOPNode *line = [LOPNode nodeWithString:@"─"];
    line->_x = x;
    line->_y = parent->_y;
    [curRow addObject:line];
}

- (LOPNode *)_addLineNode:(NSMutableArray *)curRow nextRow:(NSMutableArray *)nextRow
              parent:(LOPNode *)parent child:(LOPNode *)child {
    if (!child) return nil;
