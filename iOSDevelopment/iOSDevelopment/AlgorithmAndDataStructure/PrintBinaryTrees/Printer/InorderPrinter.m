...
+ (void)initialize {
    ...
    dispatch_once(&onceToken, ^{
        int length = 2;
        rightAppend = [NSString stringWithFormat:@"┌%@", [@"─" m_repeat:length]];
        leftAppend = [NSString stringWithFormat:@"└%@", [@"─" m_repeat:length]];
        blankAppend = [NSString m_blank:length + 1];
        lineAppend = [NSString stringWithFormat:@"│%@", [NSString m_blank:length]];
    });
}
...
- (NSString *)_printString:(id)node
    ...
    if (right) {
        [rightPrefix appendString:[NSString m_blank:length]];
    ...
    if (left) {
        [leftPrefix appendString:[NSString m_blank:length]];
...
- (NSString *)printString {
    ...
    [string appendString:[self _printString:[self.tree rootNode]
